...
//        mFetchBtn = (Button) findViewById(R.id.btn_fetch_list);
//        mFetchBtn.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                SocketClientManager.getInstance().sendBytes(new byte[]{(byte) 0xFC});
//            }
//        });
    ...
    private void startUpload() {
        ...
        SocketClientManager.getInstance().sendBytes(data);
    ...
    private void startRecycle(final LinkedList<DeviceInfo> devices) {
        if (devices != null && !devices.isEmpty()) {
            ...
            mRecycleManager.setCallback(new RecycleManager.RecycleCallback() {
                ...
                @Override
                public void onRecycling(DeviceInfo device) {
                    device.setStatus(1);
                    mResultAdapter.notifyDataSetChanged();
                }

                @Override
                public void onRecycleFailed(DeviceInfo device) {
                    device.setStatus(3);
                    mResultAdapter.notifyDataSetChanged();
                }
                ...
                @Override
                public void onFetchRate(DeviceInfo device, byte[] rate) {
                    if (rate != null) {
                        device.setRate(rate);
                        mResultAdapter.notifyDataSetChanged();
                    }
                ...
                @Override
                public void onFetchStepAndCal(DeviceInfo device, byte[] step, byte[] cal) {
                    if (step != null) {
                        device.setStep(step);
                    }
                    if (cal != null) {
                ...
                @Override
                public void onRecycleFinish(DeviceInfo device) {
                    device.setStatus(2);
                    mResultAdapter.notifyDataSetChanged();
